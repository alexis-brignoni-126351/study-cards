...
| `/study check-prereqs session-N` | Verify prerequisite concepts are at required mastery before starting a session. |
| `/study connect <card1> <card2>` | Link two cards as related concepts. |
| `/study retire <card>` | Archive a card that no longer meets the Card Inclusion Bar. |
